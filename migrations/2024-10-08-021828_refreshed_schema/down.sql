-- Note: no effort is made to retain data during this migration.

DROP TABLE project;
DROP TABLE scm_commit;
DROP TABLE test_case;
DROP TABLE commit_test_case;
DROP TABLE test_case_execution;
DROP TABLE test_case_file_covered;
DROP TABLE test_case_function_covered;
DROP TABLE denormalized_coverage_map;
DROP TABLE denormalized_coverage_map_test_case;
DROP TABLE denormalized_coverage_map_test_case_file_covered;
DROP TABLE denormalized_coverage_map_test_case_function_covered;

CREATE TABLE coverage_data (
  commit_sha TEXT NOT NULL PRIMARY KEY,

  -- FIXME: would like this to be 'jsonb', but Diesel gives error "Unsupported type: jsonb".  I guess it introspects the
  -- DB after a migration to generate src/schema.rs.  I can live with this being text for now, but it could be fun to
  -- enhance Diesel in the future.
  raw_coverage_data TEXT NOT NULL
);
