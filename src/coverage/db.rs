...
use diesel::{
    connection::{Connection, Instrumentation, SimpleConnection},
    prelude::*,
...
pub struct DieselCoverageDatabase<
    ...
    DBProvider,
> where
    Backend: diesel::backend::Backend + QueryFragment<Backend>,
    DB: Connection<Backend = Backend> + SimpleConnection + MigrationHarness<Backend>,
    DBProvider: Fn() -> Result<DB> + ?Sized,
{
    // database_url: String,
    connection_fn: Box<DBProvider>,
    connection: Option<DB>,
    test_identifier_type: PhantomData<TI>,
    coverage_identifier_type: PhantomData<CI>,
    backend_type: PhantomData<Backend>,
}

pub fn new_sqlite_from_default_path<TI: TestIdentifier, CI: CoverageIdentifier>(
) -> DieselCoverageDatabase<TI, CI, Sqlite, SqliteConnection, impl Fn() -> Result<SqliteConnection>>
{
...
pub fn new_sqlite<TI: TestIdentifier, CI: CoverageIdentifier>(
    path: &str,
) -> DieselCoverageDatabase<TI, CI, Sqlite, SqliteConnection, impl Fn() -> Result<SqliteConnection>>
{
    let path = String::from(path);
    let get_sqlite_connection = move || {
        // Create a new connection if it doesn't exist
        let mut connection =
            SqliteConnection::establish(&path).context("connecting to the database")?;
    ...
    DieselCoverageDatabase {
        ...
        connection_fn: Box::new(get_sqlite_connection),
        connection: None,
...
// fn get_sqlite_connection() -> Result<&mut SqliteConnection> {
...
impl<TI, CI, Backend, DB, DBProvider> DieselCoverageDatabase<TI, CI, Backend, DB, DBProvider>
where
    TI: TestIdentifier + Serialize + DeserializeOwned,
    CI: CoverageIdentifier + Serialize + DeserializeOwned,
    Backend: diesel::backend::Backend + QueryFragment<Backend>,
    DB: Connection<Backend = Backend> + SimpleConnection + MigrationHarness<Backend>,
    DBProvider: Fn() -> Result<DB>,
{
    // pub fn new_sqlite_from_default_path() -> DieselCoverageDatabase<TI, CI, DB> {
    //     // FIXME: hard-coded... ideally this could be overriden by CLI and ENV var and use different DBs, but this is a stopgap
    ...
    // pub fn new_sqlite(path: &str) -> DieselCoverageDatabase<TI, CI, DB, DBProvider> {
    //     DieselCoverageDatabase {
    //         database_url: String::from(path),
    //         connection: None,
    //         test_identifier_type: PhantomData,
    ...
    fn get_connection(&mut self) -> Result<&mut DB> {
        // Check if the connection already exists
        if self.connection.is_none() {
            // Create a new connection if it doesn't exist
            let mut connection = (self.connection_fn)()?;
...
impl<
        ...
        DB,
        DBProvider,
    > CoverageDatabase<TI, CI> for DieselCoverageDatabase<TI, CI, Backend, DB, DBProvider>
where
    Backend: diesel::backend::Backend + QueryFragment<Backend>,
    DB: Connection<Backend = Backend> + SimpleConnection + MigrationHarness<Backend>,
    DBProvider: Fn() -> Result<DB>,
...
mod tests {
    ...
    fn has_any_coverage_data_false() {
        let mut db = new_sqlite::<RustTestIdentifier, RustCoverageIdentifier>(":memory:");
