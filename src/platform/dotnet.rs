        ...
        for line in stdout.lines() {
            if found_intro {
                ...
                if line.starts_with(' ') {
                    ...
                    Self::parse_profiling_data(test_case, &path, dep_map, &mut coverage_data)?;
                }
            }
            ...
            get_default(move |dispatcher| {
                ...
                pool.execute(move || {
                    dispatcher::with_default(&dispatcher, || {
                        tx.send(Self::run_test(&tc, &tmp_path, &b, &dep_map))
                            .unwrap();
                    });
                });
            });
...
mod tests {
    ...
    use crate::platform::dotnet::{
        DependencyType, DotnetTestPlatform, PackageLock, PackageName, ProjectFile,
        TargetFrameworkName,
    };
