...
impl PwResponseHeader {
    ...
    pub fn parse(buf: &[u8]) -> Result<Self> {
        ensure!(buf.len() >= 36, "request body too small: {}", buf.len());

        let version = buf[0..4].try_into().map(i32::from_ne_bytes)?;
        ensure!(version == VERSION, "wrong protocol version {version}");

        let found = buf[4..8].try_into().map(i32::from_ne_bytes)?;
        let pw_name_len = buf[8..12].try_into().map(i32::from_ne_bytes)?;
        let pw_passwd_len = buf[12..16].try_into().map(i32::from_ne_bytes)?;
        let pw_uid = buf[16..20].try_into().map(u32::from_ne_bytes)?;
        let pw_gid = buf[20..24].try_into().map(u32::from_ne_bytes)?;
        let pw_gecos_len = buf[24..28].try_into().map(i32::from_ne_bytes)?;
        let pw_dir_len = buf[28..32].try_into().map(i32::from_ne_bytes)?;
        let pw_shell_len = buf[32..36].try_into().map(i32::from_ne_bytes)?;
