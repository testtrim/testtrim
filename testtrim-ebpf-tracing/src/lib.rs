...
impl EbpfTracer {
    ...
    /// This internal loop will monitor data flying out of the eBPF RingBufs and distribute it to monitored process
    /// systems.  It will be shut down when the EbpfTracer struct is dropped.
    async fn background_loop(
        mut trace_events_fd: AsyncFd<RingBuf<MapData>>,
        mut monitor_events_fd: AsyncFd<RingBuf<MapData>>,
    ...
    /// Graceful shutdown which waits for background task to cleanup.  Not critical to use this; drop will also cleanup
    /// resources through a task abort.
    pub async fn shutdown(mut self) -> Result<(), EbpfTracerError> {
        if let Some(shutdown_tx) = self.shutdown_tx.take() {
            let _ = shutdown_tx.send(true); // best effort
        }
...
mod tests {
    ...
    use crate::EbpfTracer;

    #[tokio::test]
    async fn create_and_shutdown() -> Result<()> {
